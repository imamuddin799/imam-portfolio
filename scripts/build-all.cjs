...
//         var deps = {};
//         if (pkg.dependencies) {
//             Object.keys(pkg.dependencies).forEach(function (k) { deps[k] = 1; });
//         }
//         if (pkg.devDependencies) {
//             Object.keys(pkg.devDependencies).forEach(function (k) { deps[k] = 1; });
//         }
//         return !!deps["react"];
//     } catch (e) {
//         return false;
...
//     var encodedSlug = encodeURIComponent(slugStr);
//     var assetBase = "/api/demo-react-asset?slug=" + encodedSlug + "&file=";
//     html = html.replace(/(src|href)="\/assets\//g, '$1="' + assetBase + "assets/");
//     html = html.replace(/url\(\/assets\//g, "url(" + assetBase + "assets/");
//     fs.writeFileSync(indexPath, html, "utf-8");
//     log("  Patched asset paths in index.html");
// }
...
//         if (skip.indexOf(entry.name) !== -1) return;
//         var fullPath = path.join(rootDir, entry.name);
//         var slug = slugParts.concat([entry.name]);
//         if (isReactProject(fullPath)) {
//             results.push({ fullPath: fullPath, slug: slug });
//         } else {
//             var sub = findReactProjects(fullPath, slug);
//             sub.forEach(function (p) { results.push(p); });
//         }
//     });
//     return results;
// }
...
//     var slugStr = slug.join("/");
//     var destDir = path.join(OUTPUT_ROOT, slug.join(path.sep));
...
//     var distPath = path.join(fullPath, "dist");
//     var buildPath = path.join(fullPath, "build");
//     var outPath = null;
...
// var targetSlug = process.argv[2] || null;
// var allProjects = findReactProjects(PROJECTS_ROOT, []);
...
// var toProcess = allProjects;
// if (targetSlug) {
//     toProcess = allProjects.filter(function (p) {
//         return p.slug.join("/") === targetSlug;
//     });
...
// toProcess.forEach(function (p) {
//     var success = buildProject(p.fullPath, p.slug);
//     if (success) built++;
//     else failed++;
//     console.log("");
// });
